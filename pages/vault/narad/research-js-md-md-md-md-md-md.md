---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/vault/narad/research-js-md-md-md-md-md.md"
project: "narad"
role: service
language: markdown
frameworks: [docker]
lines: 297
size: 7170 bytes
last_modified: "2026-04-09 16:48"
scanned: "2026-04-09 16:48"
tags: [docker, documentation, markdown, project/narad, service]
---

# research-js-md-md-md-md-md.md

> Service / API client module using **docker** (297 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/vault/narad/research-js-md-md-md-md-md.md` |
| **Role** | service |
| **Language** | markdown |
| **Frameworks** | docker |
| **Lines** | 297 |
| **Size** | 7170 bytes |
| **Modified** | 2026-04-09 16:48 |

## 🔗 Related Files

—

## 📄 Content

```markdown
---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/vault/narad/research-js-md-md-md-md.md"
project: "narad"
role: service
language: markdown
frameworks: [docker]
lines: 259
size: 6392 bytes
last_modified: "2026-04-09 16:07"
scanned: "2026-04-09 16:07"
tags: [docker, documentation, markdown, project/narad, service]
---

# research-js-md-md-md-md.md

> Service / API client module using **docker** (259 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/vault/narad/research-js-md-md-md-md.md` |
| **Role** | service |
| **Language** | markdown |
| **Frameworks** | docker |
| **Lines** | 259 |
| **Size** | 6392 bytes |
| **Modified** | 2026-04-09 16:07 |

## 🔗 Related Files

—

## 📄 Content

```markdown
---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/vault/narad/research-js-md-md-md.md"
project: "narad"
role: service
language: markdown
frameworks: [docker]
lines: 221
size: 5623 bytes
last_modified: "2026-04-09 15:18"
scanned: "2026-04-09 15:19"
tags: [docker, documentation, markdown, project/narad, service]
---

# research-js-md-md-md.md

> Service / API client module using **docker** (221 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/vault/narad/research-js-md-md-md.md` |
| **Role** | service |
| **Language** | markdown |
| **Frameworks** | docker |
| **Lines** | 221 |
| **Size** | 5623 bytes |
| **Modified** | 2026-04-09 15:18 |

## 🔗 Related Files

—

## 📄 Content

```markdown
---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/vault/narad/research-js-md-md.md"
project: "narad"
role: service
language: markdown
frameworks: [docker]
lines: 183
size: 4863 bytes
last_modified: "2026-04-09 14:45"
scanned: "2026-04-09 14:45"
tags: [docker, documentation, markdown, project/narad, service]
---

# research-js-md-md.md

> Service / API client module using **docker** (183 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/vault/narad/research-js-md-md.md` |
| **Role** | service |
| **Language** | markdown |
| **Frameworks** | docker |
| **Lines** | 183 |
| **Size** | 4863 bytes |
| **Modified** | 2026-04-09 14:45 |

## 🔗 Related Files

—

## 📄 Content

```markdown
---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/vault/narad/research-js-md.md"
project: "narad"
role: service
language: markdown
frameworks: [docker]
lines: 145
size: 4112 bytes
last_modified: "2026-04-09 14:38"
scanned: "2026-04-09 14:39"
tags: [docker, documentation, markdown, project/narad, service]
---

# research-js-md.md

> Service / API client module using **docker** (145 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/vault/narad/research-js-md.md` |
| **Role** | service |
| **Language** | markdown |
| **Frameworks** | docker |
| **Lines** | 145 |
| **Size** | 4112 bytes |
| **Modified** | 2026-04-09 14:38 |

## 🔗 Related Files

—

## 📄 Content

```markdown
---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/vault/narad/research-js.md"
project: "narad"
role: service
language: markdown
frameworks: [docker]
lines: 107
size: 3370 bytes
last_modified: "2026-04-09 13:31"
scanned: "2026-04-09 13:31"
tags: [docker, documentation, markdown, project/narad, service]
---

# research-js.md

> Service / API client module using **docker** (107 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/vault/narad/research-js.md` |
| **Role** | service |
| **Language** | markdown |
| **Frameworks** | docker |
| **Lines** | 107 |
| **Size** | 3370 bytes |
| **Modified** | 2026-04-09 13:31 |

## 🔗 Related Files

—

## 📄 Content

```markdown
---
source: "/home/runner/work/narad/narad/sync_temp/narad/pages/services/research.js"
project: "narad"
role: service
language: javascript
frameworks: [docker]
lines: 69
size: 2637 bytes
last_modified: "2026-04-08 16:51"
scanned: "2026-04-08 16:51"
tags: [code, docker, javascript, project/narad, service]
---

# research.js

> Service / API client module using **docker** (69 lines).

## 📋 Metadata

| Property | Value |
|----------|-------|
| **Path** | `narad/pages/services/research.js` |
| **Role** | service |
| **Language** | javascript |
| **Frameworks** | docker |
| **Lines** | 69 |
| **Size** | 2637 bytes |
| **Modified** | 2026-04-08 16:51 |

## 🔗 Related Files

[[search-js]]

## 📄 Content

```javascript
/**
 * Research Service - Implements the 'last30days' skill logic.
 * Aggregates information from various sources and synthesizes a briefing.
 */

import { webSearch } from './search.js';

export async function runLast30DaysResearch(env, topic) {
    console.log(`[Research] Running 'last30days' for: ${topic}`);
    
    // 1. Concurrent searches across different platform contexts
    const platforms = [
        { name: 'Reddit', query: `site:reddit.com ${topic} "last 30 days"` },
        { name: 'Hacker News', query: `site:news.ycombinator.com ${topic}` },
        { name: 'X / Twitter', query: `site:twitter.com ${topic} news` },
        { name: 'YouTube', query: `${topic} latest review 2024` }
    ];

    const searchResults = await Promise.all(platforms.map(async (p) => {
        try {
            const results = await webSearch(p.query, { 
                provider: 'serper', 
                limit: 5,
                apiKey: env.SERPER_API_KEY 
            });
            return { platform: p.name, data: results };
        } catch (e) {
            return { platform: p.name, error: e.message };
        }
    }));

    // 2. Synthesize with AI
    if (!env.AI) {
        return { 
            topic, 
            summary: "AI Synthesizer not available. Here are raw results.",
            results: searchResults 
        };
    }

    const contextText = searchResults.map(r => {
        if (r.error) return `[${r.platform}] Error: ${r.error}`;
        const snippets = r.data.results?.map(res => `- ${res.title}: ${res.snippet}`).join('\n') || 'No results';
        return `### ${r.platform}\n${snippets}`;
    }).join('\n\n');

    const systemPrompt = `You are the Narad Research Intelligence. 
    Analyze the provided search results for the topic "${topic}" from the last 30 days.
    Synthesize a high-fidelity briefing including:
    1. **Key Trends**: What is the community actually talking about?
    2. **Grounded Synthesis**: A narrative summary with citations to the search results.
    3. **Sentiment**: Community consensus (positive/negative/skeptical).
    4. **Actionable Insights**: How should an engineer or researcher react to this?
    
    Use a professional, brutally efficient tone.`;

    const response = await env.AI.run(env.PRIMARY_MODEL || 'llama-3.3-70b-versatile', {
        messages: [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: `Search Context from last 30 days:\n${contextText}` }
        ]
    });

    return {
        topic,
        briefing: response.response || response.text || response,
        raw_context: contextText
    };
}

```

```

```

```

```

```

```
