...
resource "null_resource" "narad_deploy" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "  sudo systemctl restart narad || (echo '✗ Restart failed' && sudo journalctl -u narad -n 50 --no-pager && exit 1)",
      "else",
      "  echo 'Starting Narad...'",
      "  sudo systemctl start narad || (echo '✗ Start failed' && sudo journalctl -u narad -n 50 --no-pager && exit 1)",
      "fi",
      "sleep 5",
      "if sudo systemctl is-active --quiet narad; then",
      "  echo '✓ Narad is running'",
      "else",
      "  echo '✗ Narad failed to start — printing last 50 log lines:'",
      "  sudo journalctl -u narad -n 50 --no-pager",
      "  exit 1",
      "fi"
    ]
  }
